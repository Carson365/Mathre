...
namespace Mathre
{
	public partial class Frm19aSecretMessage : Form, IManager
	{
		...
		public void Input(object sender, EventArgs e)
		{
			if (!decodemode)
			{
				...
				while (einput == "")
				{
					einput = Interaction.InputBox("What is your message?", "Secret Message Encoder", "");
				}
				SwapEnabled();
			}
			else
		...
		public void Output(object sender, EventArgs e)
		{
			...
			if (!decodemode)
			{
				foreach (char c in einput)
				{
					((Label)formb.Controls.Find("lblCode", true)[0]).Text += $"{c} = {(int)c}\n";
					height += 15;
				}
			}
			else
